...
class Student:
    ...
    def __str__(self):
        return f"Студент {self.number} {self.fio}, {self.bdate}," \
               f" стипендия: {self.salary}, на расслабоне: {'да' if self.on_leave else 'нет'}"
...
class Department:
    ...
    def __str__(self):
        return f"Группа 195: {self.title}, Вожак стаи: Гришакова Татьяна Николаевна, Количество студентов: {len(self.employees)} "
...
emp_1 = Student(1, 'Рудаков Михаил Сергеевич', '11.08.2003', 530)
emp_2 = Student(2, 'Алибеков Муста Магомедэминович', '12.04.2004', 1440)
...
depart_1 = Department('Стая студентов', emp_1)
